...
def fetch_cuisine_recipes(cuisine: str, limit: int = 10) -> list[dict]: #define a function that fetches full recipe details for a given cuisine from TheMealDB API with a maximum number of results (default: 10).
    ...
    stubs      = filter_by_cuisine(cuisine)[:limit] #filter recipes by cuisine and take only the first "limit" entries (stubs = partial data) 
    full_meals = [] #empty list that will store the fully detailed recipes 
    for stub in stubs: #interate over each stub (contains only basic info like ID and name). Each stub is an incomplete recipes object. 
        meal = get_meal_by_id(stub["idMeal"]) #fetch the full recipes details using its ID. 
        #Add the recipe to the list only if the result is valid (not None) 
        if meal:
            full_meals.append(meal)
    return full_meals #retun the list of fully detailed recipes for the given cuisine. 


with tab_cuisine:
    cuisines = list_cuisines() #fetches the list of all available cuisines from TheMealDB API.
    #if the cuisines list is empty or failed to load, show an error message prompting the user to check their internet connection. 
    if not cuisines:
        empty_state("Cuisine list couldn't be loaded — check your internet.")
    #if the cuisine list is available, show the interactive world map and allow users to click on a country to explore its traditional recipes.
    else:
        st.subheader("🌍 Bites Across Borders") #display the section title 
        st.markdown("- Click on a country on the map to explore its traditional recipes") #intructions 
        st.markdown("- Not sure where a country is? You can also use the selector below!") #instructions 

        from streamlit_plotly_events import plotly_events #import the library that enables click events on Potly charts in Streamlit. 

        #Render the interactive world map and capture any point the user click on the map. 
        clicked_points = plotly_events(
            build_figure(), #build and pass the Plotly world map figure. 
            click_event=True, #enable click detection on the map. 
            hover_event=False, #disable hover events to improve performance. 
            select_event=False, #disable selection events, not needed here. 
            override_width="100%", #make the map stretch to full contaner width. 
            override_height=450, #set the map height to 450 pixels. 
            key="world_map", #unique key to indentify this component in Streamlit. 
        )
